...
#if true //CODE_ANALYSIS // Define this for building on WINDOWS
...
namespace Sample
{
    ...
    public class Program
    {
        ...
        {
            var acceptTask = System.Threading.Tasks.Task.Factory.StartNew<ServerStream>(() => { return new ServerStream(port); });
            using (var stream = new ClientStream(server, port))
            {
                var svrStream = new System.Net.Security.Tests.UnixGssFakeNegotiateStream(acceptTask.Result);
                var negoStream = new MockNegotiateStream(stream);
                var isNtlm = true; // String.IsNullOrEmpty(target);
...
#else
                //this worked! if (isNtlm) protectionLevel = MockProtection.None;
#endif
#endif
                var svrTask = svrStream.AuthenticateAsServerAsync();
                ...
                svrTask.Wait();
                MockLogging.PrintInfo(null, "____Connected to " + negoStream.RemoteIdentity.Name + " (" + negoStream.RemoteIdentity.AuthenticationType + ")");
                isNtlm = String.Equals(negoStream.RemoteIdentity.AuthenticationType, "NTLM", StringComparison.OrdinalIgnoreCase);
                var sendBuf = Encoding.UTF8.GetBytes("Hello " + (isNtlm ? "NTLMv2" : "SPNEGO") + "\n");
                svrTask = svrStream.PollMessageAsync(1);
        ...
        private static void PrintUsage(string[] args)
        {
            ...
            MockLogging.PrintInfo(null, "If targetname doesn't contain '@', NTLM auth is attempted\n");
            MockLogging.PrintInfo(null, "For Kerberos auth, note the foll: \n");
            MockLogging.PrintInfo(null, "\t On Unix, username has to be in form user@REALM \n");
            MockLogging.PrintInfo(null, "\t On Windows, username and password are ignored \n");
            MockLogging.PrintInfo(null, "\nPort is hardcoded to 4433 (0x1151) \n");
        }
